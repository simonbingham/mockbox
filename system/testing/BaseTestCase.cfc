...
---------------------------------------------------------------------->
<cfcomponent extends="mockbox.system.testing.compat.framework.TestCase" 
			 output="false" 
	...
	<cffunction name="beforeTests" hint="The main setup method for running ColdBox Integration enabled tests" output="false">
		...
		if( this.loadColdbox ){
			...
			if( structKeyExists(application,getColdboxAppKey()) ){
				...
				instance.controller = CreateObject("component", "mockbox.system.testing.mock.web.MockController").init( appRootPath=appRootPath, appKey=instance.coldboxAppKey );
...
<!------------------------------------------- HELPERS ------------------------------------------->
	...
	<cffunction name="getMockRequestBuffer" access="private" output="false" returnType="mockbox.system.core.util.RequestBuffer" hint="I will return to you a mock request buffer object used mostly in interceptor calls">
	    <cfscript>
			return getMockBox().createMock("mockbox.system.core.util.RequestBuffer").init();
		</cfscript>
	</cffunction>
	
	<!--- getMockController --->
	<cffunction name="getMockController" access="private" output="false" returnType="mockbox.system.testing.mock.web.MockController" hint="I will return a mock controller object">
	    <cfscript>
			return CreateObject("component", "mockbox.system.testing.mock.web.MockController").init('/unittest','unitTest');
		</cfscript>
	</cffunction>
	...
	<cffunction name="getMockRequestContext" output="false" access="private" returntype="mockbox.system.web.context.RequestContext" hint="Builds an empty functioning request context mocked with methods via MockBox.  You can also optionally wipe all methods on it.">
		...
		<cfscript>
			...
			if( arguments.clearMethods ){
				...
				return getMockBox().createEmptyMock("mockbox.system.web.context.RequestContext");
			}
			
			// Create functioning request context
			mockRC 			= getMockBox().createMock("mockbox.system.web.context.RequestContext");
			mockController = CreateObject("component", "mockbox.system.testing.mock.web.MockController").init('/unittest','unitTest');
    ...
    <cffunction name="getMockPlugin" output="false" access="private" returntype="any" hint="Get a plugin mocked with capabilities">
    	...
    	<cfscript>
			...
			var mockWireBox   		= mockBox.createEmptyMock("mockbox.system.ioc.Injector");
