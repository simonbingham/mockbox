...
component{
	...
	variables.$mockBox = this.$mockBox 	= new mockbox.system.testing.MockBox();
	// Assertions object
	variables.$assert = this.$assert = new mockbox.system.testing.Assertion();
	// Custom Matchers
	this.$customMatchers 		= {};
	// Utility object
	this.$utility 				= new mockbox.system.core.util.Util();
	...
	*/
	remote function runRemote( string testSpecs="", string testSuites="", boolean debug=false, string reporter="simple" ) output=true{
		var runner = new mockbox.system.testing.TestBox( bundles="#getMetadata(this).name#", reporter=arguments.reporter );
